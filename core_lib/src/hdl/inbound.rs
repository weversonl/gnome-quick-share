...
impl InboundRequest {
    pub fn cleanup_partial_files(&mut self) {
        for file_info in self.state.transferred_files.values_mut() {
            if file_info.bytes_transferred >= file_info.total_size {
                continue;
            }

            let _ = file_info.file.take();
