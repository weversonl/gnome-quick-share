...
pub struct OutboundRequest {
    ...
    cancel_receiver: Receiver<String>,
    last_progress_ui_update: Instant,
    last_progress_ui_bytes: u64,
    payload: OutboundPayload,
}

impl OutboundRequest {
    async fn check_for_cancel(&mut self) -> Result<bool, anyhow::Error> {
        loop {
            match self.cancel_receiver.try_recv() {
                ...
                Err(tokio::sync::broadcast::error::TryRecvError::Closed) => return Ok(false),
    ...
    pub fn new(
        ...
        let initial_meta = match &payload {
            ...
            OutboundPayload::Text(_) => TransferMetadata {
                id: String::from(""),
                source: Some(rdi),
                ..Default::default()
            },
        ...
        Self {
            ...
            state: InnerState {
                ...
                transfer_metadata: Some(initial_meta),
                ..Default::default()
            },
            sender,
            receiver,
            cancel_receiver,
            last_progress_ui_update: Instant::now(),
            last_progress_ui_bytes: 0,
            payload,
        }
    }
    ...
    pub async fn handle(&mut self) -> Result<(), anyhow::Error> {
        ...
        tokio::select! {
            cancel = self.cancel_receiver.recv() => {
                match cancel {
                    Ok(cancel_id) => {
                        info!(
                            "outbound cancel recv: current_transfer_id={} received_cancel_id={}",
                            self.state.id,
                            cancel_id
                        );
                        if cancel_id == self.state.id {
                    ...
                    Err(e) => {
                        error!("outbound cancel channel error: {}", e);
                        return Ok(());
                    }
                }
            },
    ...
    async fn process_paired_key_result(
        ...
        match &self.payload {
            ...
            OutboundPayload::Text(text) => {
                ...
                let payload_id = rand::rng().random::<i64>().abs();
                let tmeta = TextMetadata {
                    payload_id: Some(payload_id),
                    r#type: Some(text_metadata::Type::Text.into()),
                    size: Some(bytes.len() as i64),
                    ..Default::default()
                };
                text_metadata.push(tmeta);
                total_to_send = bytes.len() as u64;
                self.state.text_payload_id = Some(payload_id);
                info!("Text payload to send: {} bytes, payload_id={}", bytes.len(), payload_id);
            }
        }
        ...
        match v1_frame.connection_response.as_ref().unwrap().status() {
            sharing_nearby::connection_response_frame::Status::Accept => {
                ...
                if let OutboundPayload::Text(text) = &self.payload {
                    let text = text.clone();
                    info!("Sending text payload: {} bytes", text.len());
                    self.send_text_payload(&text).await?;
                    self.update_state(
                        |e| {
                ...
                loop {
                    ...
                    loop {
                        ...
                        let should_inform = reached_file_end
                            ...
                            || self.last_progress_ui_update.elapsed() >= PROGRESS_UPDATE_INTERVAL;
                        ...
                        if should_inform {
                            self.last_progress_ui_update = Instant::now();
    ...
    async fn send_text_payload(&mut self, text: &str) -> Result<(), anyhow::Error> {
        let pid = self
            .state
            .text_payload_id
            .ok_or_else(|| anyhow!("text_payload_id not set before send_text_payload"))?;
        ...
        let data_frame = location_nearby_connections::OfflineFrame {
            version: Some(location_nearby_connections::offline_frame::Version::V1.into()),
            v1: Some(location_nearby_connections::V1Frame {
        ...
        self.encrypt_and_send(&data_frame).await?;

        let end_frame = location_nearby_connections::OfflineFrame {
            version: Some(location_nearby_connections::offline_frame::Version::V1.into()),
            v1: Some(location_nearby_connections::V1Frame {
                r#type: Some(location_nearby_connections::v1_frame::FrameType::PayloadTransfer.into()),
                payload_transfer: Some(PayloadTransferFrame {
        ...
        self.encrypt_and_send(&end_frame).await?;
